...
| nidam-weekly-narrative | **טרם נוצרה** — יצירה דרך RemoteTrigger נחסמה (הפרומפט מכיל את מפתח ה-push); איציק יוצר ידנית ב-claude.ai/code/routines לפי `docs/routine-weekly-narrative.md` | 0 4-11 * * 6,0,1 | הסיכום המילולי של השבוע → weekly.json.narrative. שעתי שבת/ראשון/שני 07:00–14:00 IL; שער: weekOf של weekly.json == שישי האחרון ואין עדיין narrative (כלומר רץ פעם אחת, אחרי שסגירת שישי נקלטה ו-build_weekly בנה). מקורות: חמשת הניתוחים היומיים מ-git + econ.json + earnings.json |
