...
|---|---|---|---|
| nidam-daily-market-analysis | trig_01QxCeHkMGrGcXk4Uhk4mCBv | 0 3-8 * * 2-6 (שונה מ-30 3, ראו הערה 5.9.2026 למטה) | ניתוח יומי → claude_analysis.json, שעתי 06:00-11:00 IL. קורא גם data/market.json (חוזים ברגע הכתיבה) + data/earnings.json.week (דיווחי יום-יומיים אחרונים) כדי לתפוס תזוזת-סנטימנט לילית (27.8.2026: NVDA/CRWD/CRM) שהסגירה ב-daily_state עוד לא משקפת |
| ...-retry | trig_01W8FY5Nu76pEBUvBnAePBNr | 30 3-9 * * 2-6 (שונה מ-0 5) | גיבוי, שעתי 06:30-12:30 IL (מוסט 30 דק' מהראשי — ביחד כיסוי כל 30 דק', 06:00-12:30 IL). no-op אם הראשי כבר טיפל |
...
**תגלית צדדית:** טווח הימים הקיים בשתי הרוטינות (`2-6`) אינו כולל יום שני (cron 0=א' → 2-6 = ג'-ש') — כלומר גם לפני השינוי הזה, סגירת יום שישי שנקלטת רק ביום שני לא הייתה מקבלת כותרת אוטומטית בכלל. לא תוקן (לא התבקש), לשקול אם עולה שוב.
