...
| nidam-weekly-narrative | trig_016VP2ucQCPd2y4e2jbkSUyT — נוצרה 11.9.2026 **מושבתת** ועם `<PUSH-COMMAND>` במקום שורת ה-push (יצירה עם המפתח נחסמה); איציק מדביק את שורת ה-push מ-nidam-daily-market-analysis ומפעיל. מפרט: `docs/routine-weekly-narrative.md` | 0 4-11 * * 6,0,1 | הסיכום המילולי של השבוע → weekly.json.narrative. שעתי שבת/ראשון/שני 07:00–14:00 IL; שער: weekOf של weekly.json == שישי האחרון ואין עדיין narrative (כלומר רץ פעם אחת, אחרי שסגירת שישי נקלטה ו-build_weekly בנה). מקורות: חמשת הניתוחים היומיים מ-git + econ.json + earnings.json |
...
- **ארבע תוספות "מנתונים שכבר קיימים" (11.9.2026):** (1) **ציר הזמן של The Edge Meter** בראש טאב מדדים — `renderMeterTimeline` ב-app.js, מ-history.json; הציון על רצועות-מצב, ▲ ימי מכירה, ופס S&P **בפאנל נפרד** (לא ציר-Y כפול). המצב (30/60/90, רכיבים) ב-`MT` כי הטאב נבנה מחדש בכל רענון; `ResizeObserver` כי הטאב מוסתר בזמן הבנייה (רוחב 0). צבעי הסדרות `--mt-*` עברו את בודק-הפלטה של dataviz בשני המצבים. הספארקליין ברייל של הבית הוא קישור אליו. `archive_scores.py` שומר מעכשיו גם `headline` לכל יום (מ-claude_analysis.json כשה-date זהה; 22 ימים מולאו לאחור מ-git) — זה מה שמופיע בריחוף. (2) **"איך הגיבו המדווחות של אתמול"** בעמודת הדיווחים בבית — `reactions()` ב-fetch_earnings.py. **לא** מה-CSV: עמודות Latest/%Change שם הן מיום הייצוא השבועי, ועמודת `Time` מכילה בפועל את תאריך הייצוא; מועד הדיווח יושב ב-`Released` (Before Open/After Close). התגובה נמשכת חיה מ-Yahoo; מדווחת אחרי-סגירה שהציטוט שלה עדיין מיום הדיווח מסומנת `pending` ("מגיבה במסחר הבא") במקום מספר מטעה. (3) **גרף-מיני** לכל שורה בשווקים בינלאומיים מה-`spark` שכבר נשמר; מוסתר בנייד. (4) **סיכום השבוע** — `build_weekly.py`, דטרמיניסטי (חמש הכותרות + מסלול המד + S&P שבועי + ימי מכירה), בלי LLM. טריגר: היום האחרון ב-history הוא יום שישי שטרם סוכם (`_weekly_state.json`) — כלומר נבנה ונשלח לערוץ תוך 15 דק' מדחיפת הדשבורד של סוף השבוע. **הועבר (11.9.2026 אחה"צ) לטאב מדדים, מתחת לציר הזמן (`#weekly-slot`, `renderWeekly`), ומוצג כל השבוע** — תמיד השבוע השלם האחרון, מתחלף כשהבא נבנה. מתחת לאריחים: **סיכום מילולי** (`narrative`: lead/news/earnings/macro/lookahead/writtenAt) שרוטינת `nidam-weekly-narrative` כותבת בסופ"ש; `build_weekly.py` משמר narrative קיים בבנייה מחדש של אותו שבוע. `--force` לבדיקה מקומית.
