...
C:\challenge\reports\טלגרם\ נשלחת לערוץ הטלגרם — הודעה אחת: התמונה, ומתחתיה
הכיתוב "🔗 לאתר המלא" (בלי שם קובץ). זה הפורמט הסופי שאושר (17.8.2026).

הערה היסטורית: נוסה גם פורמט "כרטיס-אתר" (דף-נחיתה עם OG tags כדי שטלגרם
יציג שם+תיאור+תמונה) — טלגרם רינדר את זה לא-עקבי (משמיט את הטקסט כשהתוכן
תמונה, עוקב אחרי meta refresh), והמיתוג ממילא נמצא בתוך הגרפיקות עצמן.
הוחלט במפורש להישאר עם הפורמט הפשוט. לא לנסות שוב בלי לשאול את איציק.
